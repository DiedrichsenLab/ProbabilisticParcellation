...
def fit_models(ks, fit_datasets=['all', 'loo', 'indiv'], rest_included=False, verbose=True, indiv_on_rest_only=False, sym_from_asym=False, mname_asym=None):
    """
    ...
    for datasets in dataset_list:
        for k in ks:
            ...
            if not sym_from_asym:
                ...
                if not Path(wdir + f'/{mname_asym}.tsv').exists():
