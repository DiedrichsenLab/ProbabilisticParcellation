...
def evaluate_task(K=10, symmetric='asym', model_type=None, model_name=None,
                  t_datasets=None, return_df=False, k_merged=None, load_best=True):
    """Evaluate group and individual DCBC and coserr of all task datasets fusion
...
def evaluate_rest(K=10, symmetric='asym', model_type=None, model_name=None,
                  t_datasets=None, return_df=False, k_merged=None, load_best=True):
    """Evaluate group and individual DCBC and coserr of all dataset fusion including rest
       and any dataset training standalone on each of the datasets.
    Args:
        K: the number of parcels
        t_datasets (list): a list of test datasets
    ...
    if model_name is None:
        model_name = ['Md','Po','Ni','Ib','Wm','De','So','Hc','MdPoNiIbWmDeSoHc']
...
if __name__ == "__main__":
    ...
    for i in range(8):        
        for k in [22,26]:
            res = evaluate_rest(K=K, symmetric=sym, model_type=['03'],
                                model_name=['MdPoNiIbWmDeSoHc'], t_datasets=[T.name[i]],
                                return_df=True)
            
            fname = f'/eval_{sym}_K-{K}_rest.tsv'
            res.to_csv(wdir + fname, index=False, sep='\t')

            D = pd.concat([D, res], ignore_index=True)
    
    fname = f'/eval_all8_{sym}_K-{K}.tsv'
    D.to_csv(wdir + fname, index=False, sep='\t')
    
    
    ############# Evaluate: Clustered models #############
    wdir = model_dir + f'/Models/Evaluation/sym'
    T = pd.read_csv(base_dir + '/dataset_description.tsv', sep='\t')
    D = pd.DataFrame()
    ...
    for i in range(7):        
        for k_merged in [22,26]:
            res = evaluate_task(K=K, symmetric=sym, model_type=['03'],
                                model_name=['MdPoNiIbWmDeSo'], t_datasets=[T.name[i]],
                                return_df=True, k_merged=k_merged, load_best=False)
