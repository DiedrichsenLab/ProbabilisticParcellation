...
def make_size_subplot(prob_sym, prob_asym, labels, cmap, wta=True):
    plt.figure(figsize=(10, 10))
    plt.subplot(2, 2, 1)
    D = cl.plot_parcel_size(prob_sym, ListedColormap(cmap), labels, wta=wta, sort=False, side='L')
    plt.ylabel('Symmetric regions (L)')

    plt.subplot(2, 2, 2)
    D = cl.plot_parcel_size(prob_sym, ListedColormap(cmap), labels, wta=wta, sort=False, side='R')
    plt.ylabel('Symmetric regions (R)')

    plt.subplot(2, 2, 3)
    D = cl.plot_parcel_size(prob_asym, ListedColormap(cmap), labels, wta=wta, sort=False, side='L')
    plt.ylabel('Asymmetric regions (L)')

    plt.subplot(2, 2, 4)
    D = cl.plot_parcel_size(prob_asym, ListedColormap(cmap), labels, wta=wta, sort=False, side='R')
